...
static const char *TAG = "NODO_SENSOR_OT";
...
_Static_assert(sizeof(sensor_packet_t) <= TRANSPORT_MAX_DATA_LEN,
               "sensor_packet_t exceeds OpenThread payload limit");
...
static esp_err_t send_packet(const sensor_packet_t *packet)
{
    ...
    if (!openthread_handler_is_attached()) {
        ESP_LOGW(TAG, "Thread no adjunto, paquete %u omitido", (unsigned)packet->batch_id);
        return ESP_ERR_INVALID_STATE;
    }

    err = openthread_handler_send((const uint8_t *)packet, sizeof(*packet));
    if (err == ESP_OK) {
        ESP_LOGI(TAG,
                 "Paquete %u enviado por OpenThread. Timestamp(ms): %" PRIu32,
                 (unsigned)packet->batch_id,
                 packet->start_timestamp_ms);
        return ESP_OK;
    }

    ESP_LOGE(TAG, "Fallo envio OpenThread: %s", esp_err_to_name(err));
    return err;
}
...
void app_main(void)
{
    ...
    err = openthread_handler_init();
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "No se pudo inicializar OpenThread: %s", esp_err_to_name(err));
        return;
    }
